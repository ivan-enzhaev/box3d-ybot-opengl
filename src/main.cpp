...
SDL_AppResult SDL_AppInit(void **appState, int argc, char *argv[])
{
    ...
    *appState = app;
    ...
    float angleInRadians = 10.0f * (3.14f / 180.0f);
    ...
    capsuleBodyDef.position = (b3Vec3) { -0.5f, 10.0f, 0.0f }; // Start offset and above the box/sphere
    b3BodyId capsuleId = b3CreateBody(app->worldId, &capsuleBodyDef);

    // Box3D capsule defined by two center points of its hemispherical caps and a radius
    b3Capsule capsule = {
        .center1 = { 0.0f, -0.5f, 0.0f }, // Bottom center
        .center2 = { 0.0f, 0.5f, 0.0f },  // Top center
        .radius = 0.3f
    };
    b3ShapeDef capsuleShapeDef = b3DefaultShapeDef();
    capsuleShapeDef.density = 1.0f;
    b3CreateCapsuleShape(capsuleId, &capsuleShapeDef, &capsule);
...
SDL_AppResult SDL_AppIterate(void *appState)
{
...
#if defined(__ANDROID__)
    ImGui::SetNextWindowPos(ImVec2(130.0f, 20.0f), ImGuiCond_FirstUseEver);
#else
    ...
    // ImGui Slider for Animation Blending (0.0 = Walk, 1.0 = Run)
    static float blendFactor = 0.0f;
    // ImGui::SliderFloat("Walk <-> Run", &blendFactor, 0.0f, 1.0f);
    ImGui::Text("Walk <-> Run");
    ...
    vec3 cubeTranslation = { -1.5f, -0.2f, 0.0f };
    ...
    // Get screen dimensions and apply display density scaling
    int windowW, windowH;
...
#else
    // --- Windows, Linux, macOS, and Emscripten: Top-Right Corner ---
    // X = Right edge minus half text width minus padding
    posX = (float)windowW - (textW * 0.5f) - padding;
    // Y = Top edge minus half text height minus padding (Y=0 is bottom-left)
